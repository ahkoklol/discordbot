...
user_last_roll = {}
# Dictionary to store the join time for each user
user_join_time = {}
...
async def on_member_join(member):
    # Set a default time for new members in the user_last_roll dictionary
...
async def roll(ctx, chosen_number: int):
    ...
    # Check if the user has been in the server for at least 24 hours
    join_time = user_join_time.get(ctx.author.id, ctx.author.joined_at.replace(tzinfo=None))
    time_in_server = datetime.datetime.now() - join_time
    ...
    if not is_admin and ctx.author.id in user_last_roll:
        last_roll_time = user_last_roll[ctx.author.id]
...
async def timeout(ctx, user: discord.User):
    ...
    if user.id in user_last_roll:
        ...
        if time_difference.total_seconds() < 86400:
            time_remaining = datetime.timedelta(seconds=86400 - time_difference.total_seconds())
